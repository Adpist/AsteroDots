...
public partial struct PlayerInputSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        else
        {
            ...
            if (shooting)
            {
                ...
                {
                    SpawnBullet(ref ecb, playerData.bulletPrefab, transform.Position, transform.Up(), bulletVelocity, -45, bulletMaxSpeed, playerData.bulletLifeTime);
                    SpawnBullet(ref ecb, playerData.bulletPrefab, transform.Position, transform.Up(), bulletVelocity, 45, bulletMaxSpeed, playerData.bulletLifeTime);
                }
            }
    ...
    {
        state.EntityManager.DestroyEntity(state.EntityManager.CreateEntityQuery(typeof(AsteroidData)).ToEntityArray(Allocator.Temp));
        state.EntityManager.DestroyEntity(state.EntityManager.CreateEntityQuery(typeof(BulletData)).ToEntityArray(Allocator.Temp));
